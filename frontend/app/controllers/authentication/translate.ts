...
const VIEW_TYPE_TRANSLATE = 'TRANSLATE';

export default class AuthenticationTranslateController extends Controller {
  ...
  async onInfiniteScroll(): Promise<void> {
    ...
    const response: RestliResponseModel =
      await this.inlineTranslationApi.fetchProposalList(VIEW_TYPE_TRANSLATE, {
        newUrl,
      });
